...
void CudaElasobjMG19::InitCudaContext() {
...
void CudaElasobjMG19::BuildGraph() {
...
void CudaElasobjMG19::SelectHandles(bool tofile) {
  if (!n_layer_) return;
  handle_ids_.clear();
...
void CudaElasobjMG19::ComputeUMatrices(bool tofile) {
  if (!n_layer_) return;
  Uf_ = new SpMat;
  deletion_queue_.push_back([=]() {
    Uf_->Destroy();
  ...
  // bottom layers
  int l = n_layer_ - 1;
  Uf_->Init(3 * n_vert_, 12 * n_handle_[l]);
  ...
  for (auto r = 0; r < n_vert_; ++r) {
    ...
    for (auto d = 0; d < 3; ++d) {
      Uf_->csr_row_[3 * r + d] = i1;
      for (auto t = 0; t < 4; ++t) {
        Uf_->csr_col_[i1] = 12 * c + 4 * d + t;
        Uf_->csr_val_[i1] = (t == 3) ? 1 : verts_[v][t];
        ++i1;
      }
    }
  }
  ...
  --l;
  for (; l >= 0; --l) {
    U_[l].Init(12 * n_handle_[l + 1], 12 * n_handle_[l]);
    ...
    for (auto r = 0; r < n_handle_[l + 1]; ++r) {
      ...
      if constexpr (reordered_) c = h2m_[l][handle_[l + 1][m2h_[l + 1][r]]];
      for (auto t = 0; t < 12; ++t) {
        U_[l].csr_row_[12 * r + t] = i1;
        U_[l].csr_col_[i1] = 12 * c + t;
...
void CudaElasobjMG19::ComputeAMatrices(bool tofile) {
  real inv_dt = 1 / dt_;
  ...
  // inertia part quasi-static
  for (auto i = 0; i < n_vert_; ++i) {
    uint32_t v = i;
    if constexpr (reordered_) v = m2h_[n_layer_][i];
  ...
  if (Af_->as_LDU_ && reordered_)
    Af_->BuildGS(c2h_off_[n_layer_], n_color_[n_layer_]);
  if (!n_layer_) return;

  // the rest As
  std::map<RowCol, Mat12> block_coo2;
  for (const auto& iter : block_coo) {
    uint32_t r = iter.first.first;
    uint32_t c = iter.first.second;
    const Mat3& a33 = iter.second;
    ...
    if constexpr (reordered_) {
      i = m2h_[n_layer_][r];
      j = m2h_[n_layer_][c];
      rd = h2m_[n_layer_ - 1][handle_[n_layer_][i]];
      cd = h2m_[n_layer_ - 1][handle_[n_layer_][j]];
    }
    RowCol key2 = std::make_pair(rd, cd);
    auto iter2 = block_coo2.find(key2);
    if (iter2 == block_coo2.end())
      iter2 = block_coo2.insert(std::make_pair(key2, Mat12::Zero())).first;
    Vec4 v1 = Vec4::Unit(3);
    v1.segment<3>(0) = verts_[i];
    Vec4 v2 = Vec4::Unit(3);
    v2.segment<3>(0) = verts_[j];
    Mat4 v12 = v1 * v2.transpose();
    for (int vi = 0; vi < 3; ++vi) {
      for (int vj = 0; vj < 3; ++vj) {
        iter2->second.block<4, 4>(4 * vi, 4 * vj) += a33(vi, vj) * v12;
      }
    }
  }
  A_ = new BSMat<12, 12>[n_layer_];
  deletion_queue_.push_back([=]() {
  ...
  for (int l = n_layer_ - 1; l >= 0; --l) {
    ...
    if (A_[l].as_LDU_ && reordered_) A_[l].BuildGS(c2h_off_[l], n_color_[l]);
    if (l) {  // not the last layer
      std::map<RowCol, Mat12> tmp_coo;
      for (const auto& iter : block_coo2) {
        ...
        if constexpr (reordered_) {
          uint32_t i = m2h_[l][r];
          uint32_t j = m2h_[l][c];
          rd = h2m_[l - 1][handle_[l][i]];
          cd = h2m_[l - 1][handle_[l][j]];
        }
        const Mat12& v = iter.second;
        RowCol key = std::make_pair(rd, cd);
        auto tmp_iter = tmp_coo.find(key);
        if (tmp_iter == tmp_coo.end())
          tmp_iter = tmp_coo.insert(std::make_pair(key, Mat12::Zero())).first;
        tmp_iter->second += v;
      }
...
void CudaElasobjMG19::BuildUpdateAuxiliary() {
  ...
  for (auto i = 0; i < n_vert_; ++i) {
    uint32_t v = i;
    if constexpr (reordered_) v = m2h_[n_layer_][i];
    d2off_[v] = block_off[std::make_pair(i, i)];
  }
  uint32_t e = 0;
  for (auto t = 0; t < n_tet_; ++t)
    for (auto u = 0; u < 4; ++u)
  ...
  if (n_layer_ > 0) {
    ...
    for (auto r = 0; r < n_vert_; ++r) {
      uint32_t i = r;
      if constexpr (reordered_) i = m2h_[n_layer_][r];
      Vec4 tmp = Vec4::Unit(3);
      tmp.segment<3>(0) = verts_[i];
    ...
    BSMat<12, 12>* UtAU = &A_[n_layer_ - 1];
    ...
    for (uint32_t b = 0; b < Af_->bnnz_; ++b) {
      ...
      uint32_t cd = handle_[n_layer_][j];
      if constexpr (reordered_) {
        rd = h2m_[n_layer_ - 1][rd];
        cd = h2m_[n_layer_ - 1][cd];
      }
      // XXt
      Vec4 xi = Vec4::Unit(3);
      xi.segment<3>(0) = verts_[i];
      Vec4 xj = Vec4::Unit(3);
      xj.segment<3>(0) = verts_[j];
    ...
    // the composition of handles_ and h2m_
    // the mapping from matrix indices to matrix indices
    mfine2coarse_ = new uint32_t*[n_layer_];
    deletion_queue_.push_back(
        [=]() { DeleteArray2D(mfine2coarse_, n_layer_); });
    for (int l = n_layer_ - 1; l >= 0; --l) {
      mfine2coarse_[l] = new uint32_t[n_handle_[l + 1]];
      for (auto i = 0; i < n_handle_[l + 1]; ++i) {
        if constexpr (reordered_)
          mfine2coarse_[l][i] = h2m_[l][handle_[l + 1][m2h_[l + 1][i]]];
        else
          mfine2coarse_[l][i] = handle_[l + 1][i];
      }
    }
  }
}

void CudaElasobjMG19::BuildCollisionAuxiliary() {
  ...
  for (int l = 0; l < n_layer_ + 1; ++l) {
    ...
    if (l == n_layer_) {
      CheckCuda(
          cudaMalloc(&dcolli_hessian_[l], sizeof(real) * MAXN_COLLI * 9));
      CheckCuda(
          cudaMemset(dcolli_hessian_[l], 0, sizeof(real) * MAXN_COLLI * 9));
    } else {
      CheckCuda(
          cudaMalloc(&dcolli_hessian_[l], sizeof(real) * MAXN_COLLI * 144));
      CheckCuda(
          cudaMemset(dcolli_hessian_[l], 0, sizeof(real) * MAXN_COLLI * 144));
    }
...
void CudaElasobjMG19::ToDevice() {
  ...
  if (Af_->as_LDU_ && reordered_) Af_->GSToDevice(cusparse_handle_);
  for (int l = 0; l < n_layer_; ++l) {
    A_[l].BCooSymToDevice(cusparse_handle_);
    if (A_[l].as_LDU_ && reordered_) A_[l].GSToDevice(cusparse_handle_);
    if (A_[l].as_dense_)
      A_[l].CholToDevice(cusolverDn_handle_, cusolverDn_params_);
  }

  ddiag_add_ = new real*[n_layer_ + 1];
  for (int l = 0; l < n_layer_; ++l) {
    CheckCuda(cudaMalloc(&ddiag_add_[l], sizeof(real) * n_handle_[l] * 144));
  }
  CheckCuda(cudaMalloc(&ddiag_add_[n_layer_], sizeof(real) * n_vert_ * 9));
  deletion_queue_.push_back(
      [=]() { DeleteCudaArray2D(ddiag_add_, n_layer_ + 1); });
  if constexpr (reordered_) {
    dh2m_ = new int32_t*[n_layer_ + 1];
    for (int l = 0; l <= n_layer_; ++l) {
      CheckCuda(cudaMalloc(&dh2m_[l], sizeof(int32_t) * n_handle_[l]));
      CheckCuda(cudaMemcpy(dh2m_[l], h2m_[l], sizeof(int32_t) * n_handle_[l],
                           cudaMemcpyHostToDevice));
    }
    deletion_queue_.push_back(
        [=]() { DeleteCudaArray2D(dh2m_, n_layer_ + 1); });
    dm2h_ = new int32_t*[n_layer_ + 1];
    for (int l = 0; l <= n_layer_; ++l) {
      CheckCuda(cudaMalloc(&dm2h_[l], sizeof(int32_t) * n_handle_[l]));
      CheckCuda(cudaMemcpy(dm2h_[l], m2h_[l], sizeof(int32_t) * n_handle_[l],
                           cudaMemcpyHostToDevice));
    }
    deletion_queue_.push_back(
        [=]() { DeleteCudaArray2D(dm2h_, n_layer_ + 1); });
  }
  ...
  if (n_layer_ > 0) {
    CheckCuda(cudaMalloc(&dUtAU_diag_fix_,
                         sizeof(real) * n_handle_[n_layer_ - 1] * 144));
    ...
    dmfine2coarse_ = new int32_t*[n_layer_];
    deletion_queue_.push_back(
        [=]() { DeleteCudaArray2D(dmfine2coarse_, n_layer_); });
    for (int l = 0; l < n_layer_; ++l) {
      CheckCuda(
          cudaMalloc(&dmfine2coarse_[l], sizeof(int32_t) * n_handle_[l + 1]));
      CheckCuda(cudaMemcpy(dmfine2coarse_[l], mfine2coarse_[l],
                           sizeof(int32_t) * n_handle_[l + 1],
                           cudaMemcpyHostToDevice));
    }
    CheckCuda(
        cudaMalloc(&dAf_XXt_, sizeof(real) * red_n_half_[n_layer_ - 1] * 144));
...
void CudaElasobjMG19::FixDiag() {
  ...
  for (uint32_t i = 0; i < n_vert_; ++i) {
    ...
    if constexpr (reordered_) idx = h2m_[n_layer_ - 1][idx];
    Vec4 v1 = Vec4::Unit(3);
    v1.segment<3>(0) = verts_[i];
  ...
  UtAU_diag_fix_ = new real[n_handle_[n_layer_ - 1] * 144];
  deletion_queue_.push_back([=]() { delete[] UtAU_diag_fix_; });
  memset(UtAU_diag_fix_, 0, sizeof(real) * n_handle_[n_layer_ - 1] * 144);
  for (auto i = 0; i < n_handle_[n_layer_ - 1]; ++i) {
    for (auto c = 0; c < 3; ++c) {
      for (auto di = 0; di < 4; ++di) {
        for (auto dj = 0; dj < 4; ++dj) {
          UtAU_diag_fix_[144 * i + (c * 4 + di) * 12 + (c * 4 + dj)] =
              diag_fix[i](di, dj);
        }
      }
    }
...
void CudaElasobjMG19::ComputeShortestPath(uint32_t source, LenFrom* shortest) {
  struct LenFromTo {
    LenFrom lf;
    uint32_t to;
...
uint32_t CudaElasobjMG19::GraphColoring(uint32_t* v2e_off,
...
uint32_t CudaElasobjMG19::Coloring(uint32_t* v2e_off, uint32_t* edge_to,
...
void CudaElasobjMG19::DeleteArray2D(_type** ptr, uint32_t size) {
  if (ptr) return;
...
template <typename _type>
void CudaElasobjMG19::DeleteCudaArray2D(_type** ptr, uint32_t size) {
  if (ptr) return;
