...
namespace Rain {
void NeoHookean::SetLame(real young, real poisson) {
...
void NeoHookean::GetPiola(const Mat3& F, Mat3& P) {
...
void NeoHookean::GetMixedPiola(const Mat3& F, Mat3& P) {
  real fac = -mu_;
  P.col(0) = fac * F.col(1).cross(F.col(2));
  P.col(1) = fac * F.col(2).cross(F.col(0));
  P.col(2) = fac * F.col(0).cross(F.col(1));
...
void NeoHookean::GetdPdF(const Mat3& F, Mat9& dPdF) {
  Vec9 eigenvalues;
  Mat9 eigenvectors;
...
void NeoHookean::GetMixeddPdF(const Mat3& F, Mat9& dPdF) {
  Vec9 eigenvalues;
  Mat9 eigenvectors;
...
void NeoHookean::GetdPdFSVD(const Mat3& F, const Mat3& U, const Mat3& V,
...
void NeoHookean::GetMixeddPdFSVD(const Mat3& F, const Mat3& U, const Mat3& V,
                                 const Vec3& S, Mat9& dPdF) {
  Vec9 eigenvalues;
  Mat9 eigenvectors;
