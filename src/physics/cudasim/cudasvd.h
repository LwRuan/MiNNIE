...
__device__ __forceinline__ float rsqrt(const float f) {
  return 1.0f / sqrt(f);
}
...
__device__ __forceinline__ void svd_slow(
    float a11, float a12, float a13, float a21, float a22, float a23, float a31,
    float a32, float a33,  // input A
    float &u11, float &u12, float &u13, float &u21, float &u22, float &u23,
    float &u31, float &u32, float &u33,  // output U
...
__device__ __forceinline__ void SVD(const Mat3f &A, Mat3f &U, Mat3f &V,
                                    Vec3f &S) {
  svd(A(0, 0), A(0, 1), A(0, 2), A(1, 0), A(1, 1), A(1, 2), A(2, 0), A(2, 1),
